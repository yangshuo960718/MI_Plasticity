...
for i = 1:nFrame
    ...
    for i = 1:size(EleStress, 1)
        ...
        S11 = S11 + EleStress(i, 1) * S(i);
        S22 = S22 + EleStress(i, 2) * S(i);
        S33 = S33 + EleStress(i, 3) * S(i);
        S12 = S12 + EleStress(i, 4) * S(i);
        S13 = S13 + EleStress(i, 5) * S(i);
        S23 = S23 + EleStress(i, 6) * S(i);
    end
    ...
    frame_stress = [s11, s22, s33, s12, s13, s23];
...
%%
%load('./Plot/trace-free/pp/0d8_stress_strain.mat', 'MacroStrain', 'HomoStress');
%plot(MacroStrain, HomoStress(:, 1), '-o');
plot(MacroStrain, HomoStress(:, 2), '-o');

save('./Plot/trace-free/pp/0d4_stress_strain.mat', 'MacroStrain', 'HomoStress');
grid on;